...

{
    ...
    for(i=1;i<=pdata.supp;i++) {
		move.iter_in[i] = 0;
		move.iter_out[i] = 0;
		for(j=1;j<=pdata.supp;j++) move.iter_swap[i][j] = 0;
	}
   ...
   for (i = 1; i <= pdata.scen; ++i)	lp[i] = NULL; 
 ...
 ************************************************************/
...

 {
	 ...
	 evaluate(env,lp,iter,pdata,pplex,psol,hcode,ncoded,coded_sol,
		 coded_value);
	 form_list(pdata,psol,ins_list,del_list,swap_list,iter,move,
		 coded_sol,ncoded);
	 while(iter < max_iter) {
		 select_move(env,lp,pdata,psol,pplex,ins_list,del_list,
			 swap_list,move,ins_length,del_length,swap_length,iter,
			 coded_sol,coded_value,ncoded);
		 execute_move(env,lp,pplex,pdata,psol,move,iter);
		 form_list(pdata,psol,ins_list,del_list,swap_list,iter,
			 move,coded_sol,ncoded);
			 iter +=1;
	 }
     ftime = clock();
...

 {
	 ...
	 if(move->flag) {
		 for(i=1;i<=pdata.supp;i++) {
			...
			for(j=1;j<=pdata.scen;j++) {
				status = CPXsolution (env, lp[j], &solstat1, &objetivo, pplex->z,
						     pplex->pi, pplex->slack, pplex->dj);
				if ( status ) {
				 ...
				 *coded_sol, double *coded_value,int *ncoded)
{
	...
	for(k=1;k<=ins_list[0];k++) { 
		...
		if(!is_there(hcode,coded_sol,ind,ncoded)){
			...
			if(objetivo < move->value) {
				if( (iter >= move->iter_in[ins_list[k]]) ||
					(objetivo < psol->best_value) ) {
					move->type=1;
					move->flag = 0;
					move->deletion = 0;
    ...
    for(k=1;k<=del_list[0];k++) {    
		...
		if(!is_there(hcode,coded_sol,ind,ncoded)){
			...
			if(objetivo < move->value) {
				if( (iter >= move->iter_out[del_list[k]]) ||
					(objetivo < psol->best_value) ){
					move->type=2;
					move->flag = 0;
			   ...
			   movedata *move, _int64 *coded_sol,int *ncoded)
{
	...
	for(i=1;i<=pdata.supp;i++) {
		...
		for(j=1;j<=pdata.scen;j++) psol->exp_dual_cost[i] += pdata.prob[j]*psol->dual_cost[j][i];
		rel_ben[i] = psol->exp_dual_cost[i] / pdata.fixedcost[i];
	}

	for(i=1;i<=pdata.supp;i++) {
		for(j=1;j<=pdata.supp;j++) swap_ben[i][j] = rel_ben[j] - rel_ben[i];
	}

	/* candidates for insertion */
	for(j=1;j<=pdata.supp;j++)	ins[j] = 0;
	...
	for(i=1;i<= max_ins_length;i++){
		...
		if(jbest) ins[jbest] = 1;

		if(jbest) {
			psol->y[jbest]=1;
			hcode = code_sol(psol,pdata);
			if(is_there(hcode,coded_sol,ind,ncoded)) max_ins_length += 1;
			psol->y[jbest] = 0;
		}
	...
	for(j=1;j<=pdata.supp;j++)	del[j] = 0;
	...
	for(k=1;k<= max_swap_length;k++){
		...
		if(ibest && jbest) swap[ibest][jbest] = 1;
		
		if(ibest && jbest) {
			psol->y[jbest]=1;
			psol->y[ibest]=0; 
			hcode = code_sol(psol,pdata);
			if(is_there(hcode,coded_sol,ind,ncoded)) max_swap_length += 1;
			psol->y[jbest] = 0;
			psol->y[ibest] = 1;
			}
	}
...

{
	...
	fp = fopen(filename,"r");
	if (fp == NULL) nrerror("Unable to open input file.");

	/* read size of the problem after skipping headears */
	a = getc(fp);
	while(!isdigit(a)) a = getc(fp);
    ungetc(a, fp);
	fscanf(fp,"%d %d %d", &pdata->plants, &pdata->supp, &pdata->scen);
	...
	for (i = 1; i <= pdata->supp; ++i)
		for (j = 1; j <= pdata->plants; ++j)
			fscanf(fp, "%lf", &pdata->trancost[i][j]);
    ...
    ungetc(a, fp);
	for (i = 1; i <= pdata->supp; ++i)
		fscanf(fp, "%d %lf", &pdata->cap[i], &pdata->fixedcost[i]);
...
void initialize(inputdata pdata, soldata *psol)
{
    ...
    capacity = 0;
        ...
        psol->itime = clock();
	for(i=1;i<=pdata.supp;i++) psol->y[i]=0;
	...
	for(i=1;i<=pdata.supp;i++) psol->ybest[i]=psol->y[i];
}
				...
				int *ncoded,_int64 *coded_sol,double *coded_value)
{
	...
	for(j=1;j<=pdata.scen;j++) {
		...
		for(i=1;i<=pdata.supp;i++) psol->dual_cost[j][i] = pplex->pi[i];
		trans[j] = objetivo;
		fixed[j] = 0;
		for(i=1;i<=pdata.supp;i++) {
			fixed[j] += pdata.exch[i][j]*pdata.fixedcost[i]*psol->y[i];

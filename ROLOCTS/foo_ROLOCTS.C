...
void search(inputdata pdata,soldata *psol, CPXENVptr env, 
			CPXLPptr *lp, datacplex *pplex,movedata *move, char * out, double lambda);
...
void select_move(CPXENVptr env,CPXLPptr *lp,inputdata pdata,soldata *psol,
				 ...
				 *coded_sol, double *coded_value,int *ncoded, double lambda);
void execute_move(CPXENVptr env,CPXLPptr *lp,datacplex *pplex,
				  inputdata pdata,soldata *psol,movedata *move, int iter, char * out, double lambda);
...
void write_output_foo2(char *salida, soldata *psol, double t1, double t2, double lambda);
...
int main(int argc,char **argv)
{
   ...
   search(pdata, &psol, env, lp, &pplex, &move, out, lambda);
 ...
 ************************************************************/
			...
			CPXLPptr *lp, datacplex *pplex, movedata *move, char * out, double lambda)
 {
	 ...
	 while(iter < max_iter) 
	 {
		 select_move(env, lp, pdata, psol, pplex, ins_list, del_list, swap_list, move, ins_length, del_length, swap_length, iter, coded_sol, coded_value,ncoded, lambda);
		 execute_move(env, lp, pplex, pdata, psol, move, iter, out, lambda);
	 ...
	 write_output_foo2(out, psol, etime, ttime, lambda);
 ...
 void execute_move(CPXENVptr env,CPXLPptr *lp,datacplex *pplex,
		inputdata pdata,soldata *psol,movedata *move,int iter, char * out, double lambda) // se agrega apuntador con la entrada para escribir a archivo 
 {
	 int i, j;
...
void write_output_foo2(char *salida, soldata *psol, double etime, double ttime, double lambda)
{
	FILE *f2;
	errno_t err_open;
	err_open = fopen_s(&f2, salida, "a+");

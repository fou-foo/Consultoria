...
int main(int argc,char **argv)
{
	...
	input_data( in, &pdata); //se cambia 
	//INFBOUND = pdata.max_dem;
	//printf("INFBOUND %f  que cambie \n", INFBOUND);
...
int n_choose_k(int n,int k)
{
	...
	numerator = 1;
	for(i=k+1;i<=n;i++) numerator *= i;

	denominator = 1;
	for(i=2;i<=n-k;i++) denominator *= i;

...
int main(int argc, char **argv)
{
	...
	//double *rel_ben;
	incremento = 1. / max_iter; // incremento lo fijamos en función del numero de iteraciones
	...
	fclose(f2);
	for (lambda = 0.0; lambda <= 1. + incremento; ) // para cada psoible valor de lamba realizamos la busqueda que ya se habia implementado considerando como parametro adicional la combinacion convexa con \lambda
	{
		...
		all_lambda(&psol, &move, env, lp, &pplex, status, pdata, out, out_mejores, lambda, max_iter, &exp_value, &bstdv);//se escriben todas las soluciones encontradas
		lambda = lambda + incremento;
	}
	//comienza lectura y calculo de soluciones no dominadas, del archivo creado en la seccion anterior
	err_open = fopen_s(&fp, out, "r"); // declaramos un apuntador a archivo
	if (err_open != 0) nrerror("Unable to open OUT_lambdas file."); //cachamos el error de lectura
	a = getc(fp);
	while (!isdigit(a)) a = getc(fp);
	ungetc(a, fp);
	//se leen los puntos optimos desde archivo para filtrar la frontera de pareto dada por los puntos dominantes
	...
	for (i = 0; i < (max_iter)*(max_iter); i++) //recorremos todas las souliciones para buscar el omptimo de pareto y comenzar a filtrar
	{
		if (ParetoFront[i][2] <= pareto[2] && ParetoFront[i][1] <= pareto[1])//minimo en eje riesgo y expected_value
		{
	...
	{
		if (ParetoFront[i][2] <= candidato1[2])//comparamos la solucion en riesgo contra el candidato en turno
		{
			...
			if (candidato2[1] >= candidato1[1])//comparamos expected value del candidato2 
			{
				...
				// en caso contrario lo dejamos pasar y actualizamos valores
	...
	pareto[3] = ParetoFront[(max_iter)*(max_iter)][3];//para un segundo barrido iniciamos con el ultimo punto solucion encontrados (esto solo para inicializar)
	pareto[2] = ParetoFront[(max_iter)*(max_iter)][2];
	pareto[1] = ParetoFront[(max_iter)*(max_iter)][1];
	pareto[0] = ParetoFront[(max_iter)*(max_iter)][0];
	for (i ; i >=0; i--) //guardamos en el for anterior el ultimo indice revizado, por lo que comenzamos el segundo barrido desde ese indice
	{
		if (ParetoFront[i][2] <= pareto[2] && ParetoFront[i][1] <= pareto[1])//minimo en ambas partes
		{
			pareto[3] = ParetoFront[i][3];
			pareto[2] = ParetoFront[i][2];
			pareto[1] = ParetoFront[i][1];
	...
	for (i = index + 1; i >= 0; i--)
	{
		if (ParetoFront[i][1] <= candidato1[1])//comparamos expected_value
		{
			candidato2[3] = ParetoFront[i][3];
			candidato2[2] = ParetoFront[i][2];
			candidato2[1] = ParetoFront[i][1];
